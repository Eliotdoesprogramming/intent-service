...
from cli import app
from test_ml import intents_data  # Reuse the test data fixture
...
def mock_train_utils():
    with patch("cli.get_train_utils") as mock:
        ...
        train_classifier_mock = MagicMock(return_value=(MagicMock(), ["greeting", "farewell"], MagicMock()))
...
def test_register_command(mock_mlflow):
    ...
    mock_mlflow.pyfunc.load_model.side_effect = mlflow.exceptions.MlflowException("Not found")
    result = runner.invoke(
        app,
        ["register", "invalid_run_id", "test_model"]
    )
...
def test_search_command(mock_mlflow):
    ...
    type(mock_model).tags = PropertyMock(return_value={"intent_greeting": "true", "version": "1.0.0"})
...
def test_info_command(mock_mlflow):
    ...
    mock_client.get_latest_versions.return_value = [MagicMock(version="1", run_id="test_run")]
...
def test_predict_command(mock_mlflow):
    ...
    result = runner.invoke(
        app,
        ["predict", "test_model", "hello there"]
    )
    assert result.exit_code == 0
...
def test_serve_command(mock_api):
    ...
    result = runner.invoke(
        app,
        ["serve", "--environment", "prod", "--workers", "4"]
    )
    assert result.exit_code == 0
    assert "Starting API server" in result.stdout
